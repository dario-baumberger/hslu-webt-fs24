...
class Database
{
    ...
    function writeDocument($data)
    {
        ...
        $sql = 'INSERT INTO test (place_id, name, route, street_number, postal_code, locality, type) VALUES (:place_id, :name, :route, :street_number, :postal_code, :locality, :type)';
        $stmt = self::$conn->prepare($sql);
        $stmt->bindParam(':place_id', $place_id);
        $stmt->bindParam(':name', $data['name']);
        $stmt->bindParam(':route', $data['route']);
        $stmt->bindParam(':street_number', $data['street_number']);
